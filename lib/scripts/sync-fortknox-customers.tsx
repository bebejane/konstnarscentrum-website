  ...
  for (const region of regions) {
    ...
    for (const member of regionMembers) {
      try {
        ...
        await client.items.update(member.id, { fortnox_customer_number: createdCustomer.CustomerNumber })
        created++
    ...
    console.log(`[${region.name}] Done: ${created} created, ${linked} linked by email, ${updated} already linked, ${errors.length} errors`)
    if (errors.length) {
